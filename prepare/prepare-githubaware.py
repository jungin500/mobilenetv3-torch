...
from githubaware_mobilenetv3 import mobilenetv3_large
...
if __name__ == '__main__':
    ...
    model = mobilenetv3_large(num_classes=1000, width_mult=1.).to(device)
    # distributed model
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr=args.learning_rate, weight_decay=0.00004, momentum=0.9)
    ...
    for epoch in epoch_range:
        ...
        if epoch < 5:
            print("Setting LR to 4.0 for first 5 epochs!")
            for g in optimizer.param_groups:
                g['lr'] = 4.0
        elif epoch == 5:
            print("Setting LR back to 1.0!")
            for g in optimizer.param_groups:
                g['lr'] = 1.0
