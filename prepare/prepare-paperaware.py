...
if __name__ == '__main__':
    ...
    valid_dataset = ImageNet(
        labels=labels,
        root_dir='/dataset/Data/CLS-LOC/val-sub',
        transform=transforms.Compose([
    ...
    model = MobileNetV3(width_mult=0.5, classifier=True, classifier_out_features=1000).float().to(device)
    # distributed model
    # model = torch.nn.DataParallel(model)  # do not uncomment this with AMP!
